...
MySceneGraph.prototype.parseMaterials = function(materialsNode) {
  ...
  for (var i = 0; i < children.length; i++)
  {
    ...
    if(this.reader.hasAttribute(children[i], 'invert'))
    {
      inv = this.reader.getBoolean(children[i], 'invert');

      console.log("este material tem campo invert = " + inv);
    }
    /*retirar*/
    ...
    {
      newMaterial.setShininess(1 - shininess);

      newMaterial.setAmbient(1 - ambientComponent[0], 1 - ambientComponent[1], 1 - ambientComponent[2], 1 - ambientComponent[3]);

      newMaterial.setDiffuse(1 - diffuseComponent[0], 1 - diffuseComponent[1], 1 - diffuseComponent[2], 1 - diffuseComponent[3]);

      newMaterial.setSpecular(1 - specularComponent[0], 1 - specularComponent[1], 1 - specularComponent[2], 1 - specularComponent[3]);

      newMaterial.setEmission(1 - emissionComponent[0], 1 - emissionComponent[1], 1 - emissionComponent[2], 1 - emissionComponent[3]);
    }
